...
export default function MyLoans() {
    ...
    const confirmReturn = async () => {
        ...
        try {
            ...
            setMessage(error?.response?.data?.message || 'Gagal mengembalikan buku.');
        } finally {
    ...
    return (
        <section className="space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-semibold">Pinjaman Saya</h1>
                    <p className="text-sm text-slate-500 dark:text-slate-400">
                        Lacak semua buku yang sedang kamu pinjam dan status pengembaliannya.
                    </p>
                </div>
                ...
                <div className="grid gap-4">
                    {loans.length === 0 && (
                        <div className="rounded-2xl border border-dashed border-slate-300 p-6 text-center text-sm text-slate-500 dark:border-slate-700">
                            Belum ada data pinjaman di status ini.
                        </div>
                    )}
                    {loans.map((loan) => (
                        <div
                                ...
                                {loan.status === 'active' || loan.status === 'overdue' ? (
                                    <Button variant="secondary" onClick={() => handleReturn(loan.id)}>
                                        ...
                                        <ConfirmDialog
                                            ...
                                            description="Apakah Anda yakin ingin mengembalikan buku ini?"
